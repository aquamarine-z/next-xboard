...
import { HandCoins, Wallet, Loader2 } from "lucide-react";
import { dialog, SurfaceDialogContent } from "@/components/ui/surface";
import { toast } from "@/components/ui/sonner";

export interface CommissionWithdrawModalProps {
...
export function CommissionWithdrawModal({
  ...
  const [methodStr, setMethodStr] = React.useState("");
  ...
  const isValid = numCents > 0 && numCents <= maxCents && methodStr.trim().length > 3;
    ...
    try {
      const res = await fetch("/api/xboard", {
        ...
        body: JSON.stringify({
          ...
          withdraw_amount: numCents,
          withdraw_method: methodStr.trim(),
        }),
      });

      const json = await res.json();
      if (res.ok && (json.data === true || json.success)) {
        toast.success(t("invites.withdraw_success"));
        onSuccess?.();
        void close(true);
      } else {
    ...
    >
      <form onSubmit={handleSubmit}>
        <div className="pt-7 px-6 pb-2 flex flex-col items-center text-center">
          <div className="w-13 h-13 rounded-[20px] bg-gradient-to-b from-blue-500/15 to-blue-500/5 text-[#0071e3] dark:text-[#2997ff] border border-[#0071e3]/20 flex items-center justify-center mb-3 shadow-xs select-none">
            <HandCoins className="w-6 h-6 stroke-[1.8]" />
          </div>
          <h3 className="text-[19px] font-semibold text-foreground tracking-tight leading-snug">
            {t("invites.withdraw_title")}
          </h3>
          <p className="mt-1 text-xs text-muted-foreground leading-relaxed px-2">
            {t("invites.withdraw_desc")}
          </p>
        </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between px-1">
              <span className="text-[11px] font-medium text-muted-foreground">
                {t("invites.withdraw_amount")}
              </span>
              <button
                type="button"
                onClick={() => setAmountStr((maxCents / 100).toString())}
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full h-10 pl-8 pr-3 rounded-full bg-secondary/60 border border-border text-sm text-foreground placeholder:text-muted-foreground font-mono outline-hidden focus:border-[#0071e3] transition-all"
              />
            </div>
          </div>

          {/* Account/Method input */}
          <div className="space-y-1">
            <span className="text-[11px] font-medium text-muted-foreground px-1 select-none">
              {t("invites.withdraw_method")}
            </span>
            <input
              type="text"
              value={methodStr}
              onChange={(e) => setMethodStr(e.target.value)}
              placeholder={t("invites.withdraw_method_placeholder")}
              className="w-full h-10 px-3.5 rounded-full bg-secondary/60 border border-border text-xs text-foreground placeholder:text-muted-foreground outline-hidden focus:border-[#0071e3] transition-all"
            />
          </div>
        ...
        <div className="p-5 pt-2 flex items-center gap-2.5">
          ...
          <button
            ...
            className="w-2/3 h-11 rounded-full bg-gradient-to-r from-[#0071e3] to-[#0066cc] hover:from-[#0077ed] hover:to-[#005bb5] text-white font-medium text-xs flex items-center justify-center gap-1.5 shadow-xs shadow-[#0066cc]/25 transition-all ios-touch-feedback active:scale-[0.98] cursor-pointer select-none disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
              <span>{t("invites.withdraw_btn")}</span>
            )}
          </button>
        </div>
      </form>
...
export function openCommissionWithdrawDialog(
