    ...
    <div className="space-y-8 sm:space-y-10 animate-in fade-in duration-300">
      ...
      {activeArticle ? (
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          {/* Controls: Search & Category Capsules */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            {/* Search Input with Apple Frosted Capsule */}
            <div className="relative flex-1 max-w-md">
              <Search className="w-4 h-4 text-muted-foreground absolute left-3.5 top-1/2 -translate-y-1/2 opacity-70" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={t("knowledge.search_placeholder")}
                className="w-full h-10 pl-9 pr-9 text-xs sm:text-[13px] bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.08] dark:border-white/[0.08] rounded-full focus:outline-none focus:ring-2 focus:ring-[#0066cc]/25 focus:border-[#0066cc] placeholder:text-muted-foreground/70 transition-all"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            {categories.length > 0 && (
              <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
                <button
                  ...
                  className={`apple-pill-btn shrink-0 whitespace-nowrap px-4 py-1.5 text-xs transition-all active:scale-95 ${
                    selectedCategory === "all"
                      ? "bg-[#1d1d1f] text-white dark:bg-[#f5f5f7] dark:text-[#1d1d1f] font-medium shadow-xs"
                      : "bg-black/[0.04] dark:bg-white/[0.06] text-[#86868b] hover:text-foreground hover:bg-black/[0.08] dark:hover:bg-white/[0.1]"
                  }`}
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
              {filteredArticles.map((article) => (
                ...
                >
                  ...
                  <div className="pt-4 mt-4 border-t border-black/[0.05] dark:border-white/[0.06] flex items-center justify-between text-xs text-muted-foreground">
            ...
            <div className="p-14 sm:p-20 rounded-[24px] border border-dashed border-black/[0.08] dark:border-white/[0.08] text-center space-y-3.5 bg-black/[0.02] dark:bg-white/[0.02]">
              ...
              {searchQuery && (
                <button
                  ...
                  className="apple-pill-btn px-4 py-1.5 text-xs bg-[#0066cc] text-white active:scale-95"
                >
                  清除搜索
                </button>
              )}
